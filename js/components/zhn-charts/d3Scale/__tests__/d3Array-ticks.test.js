...
describe('d3Array ticks', function () {
  var fn = _d3Array.ticks;
  it("ticks(start, stop, count) returns the empty array if any argument is NaN", function () {
  ...
  it("ticks(start, stop, count) returns the empty array if start === stop and count is non-positive", function () {
    expect(fn(1, 1, -1)).toEqual([]);
    expect(fn(1, 1, 0)).toEqual([]);
    expect(fn(1, 1, NaN)).toEqual([]);
  });
  it("ticks(start, stop, count) returns the empty array if start === stop and count is positive", function () {
    expect(fn(1, 1, 1)).toEqual([1]);
    expect(fn(1, 1, 10)).toEqual([1]);
  });
  it("ticks(start, stop, count) returns the empty array if count is not positive", function () {
    expect(fn(0, 1, 0)).toEqual([]);
    expect(fn(0, 1, -1)).toEqual([]);
    expect(fn(0, 1, NaN)).toEqual([]);
  });
  it("ticks(start, stop, count) returns the empty array if count is infinity", function () {
    expect(fn(0, 1, Infinity)).toEqual([]);
  });
  it("ticks(start, stop, count) does not include negative zero", function () {
    expect(1 / fn(-1, 0, 5).pop()).toBe(Infinity);
  });
  it("ticks(start, stop, count) remains within the domain", function () {
    expect(fn(0, 2.2, 3)).toEqual([0, 1, 2]);
  });
  it("ticks(start, stop, count) returns approximately count + 1 ticks when start < stop", function () {
  ...
  it("ticks(start, stop, count) returns the reverse of ticks(stop, start, count)", function () {
  ...
  it("ticks(start, stop, count) handles precision problems", function () {
    expect(fn(0.98, 1.14, 10)).toEqual([0.98, 1, 1.02, 1.04, 1.06, 1.08, 1.1, 1.12, 1.14]);
  });
  it("ticks(start, stop, count) tries to return at least one tick if count >= 0.5", function () {
...
//# sourceMappingURL=d3Array-ticks.test.js.map