...
const CL_SECTOR = 'recharts-sector'
...
const getTangentCircle = ({
  ...
  , theta = Math.asin(cornerRadius / centerRadius) / RADIAN
  , centerAngle = cornerIsExternal
     ? angle
     : angle + sign * theta
  , center = polarToCartesian(cx, cy, centerRadius, centerAngle)
  // The coordinate of point which is tangent to the circle
  , circleTangency = polarToCartesian(cx, cy, radius, centerAngle)
  // The coordinate of point which is tangent to the radius line
  , lineTangencyAngle = cornerIsExternal
     ? angle - sign * theta
     : angle
...
const getSectorPath = ({
  ...
  endAngle
    ...
    , outerStartPoint = polarToCartesian(cx, cy, outerRadius, startAngle)
    , outerEndPoint = polarToCartesian(cx, cy, outerRadius, tempEndAngle);
    ...
    if (innerRadius > 0) {
      ...
      , innerEndPoint = polarToCartesian(cx, cy, innerRadius, tempEndAngle);
...
export const Sector = memo((props) => {  
  const {
    cx,
    cy,
    innerRadius,
  ...
  const layerClass = classNames(CL_SECTOR, className)
  , deltaRadius = outerRadius - innerRadius
  , cr = getPercentValue(cornerRadius, deltaRadius, 0, true)
  , path = cr > 0 && Math.abs(startAngle - endAngle) < 360
     ? getSectorWithCorner({
         cx,
         cy,
         innerRadius,
